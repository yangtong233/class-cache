...
public class MethodMetadata {
    ...
    private Class<?> extractElementTypeInternal() {
        ...
        // 处理泛型集合类型（List、Set等）
        if (isCollectionType(returnType) && returnGenericType instanceof ParameterizedType) {
            ParameterizedType parameterizedType = (ParameterizedType) returnGenericType;
            ...
            if (actualTypeArguments.length > 0) {
                ...
                if (elementType instanceof ParameterizedType) {
                    ParameterizedType paramElementType = (ParameterizedType) elementType;
